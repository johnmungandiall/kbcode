...
**kbcode** is a terminal-based AI coding agent (v1.14.0) that blends five ideas
...
## Key entry points
- `kbcode/cli.py:383` — `main()` entry point, parses args, dispatches to wizard/init/REPL
...
- Tests: `pytest` (33 files under `tests/`, CI in `.github/workflows/ci.yml`)

See [[cheatsheet]] for the full command list.

## Version
- `kbcode/__init__.py:9` — `__version__ = "1.15.0"`; release history in [[changelog]]

last indexed: 2026-07-02 (post-v1.15.0: auto mode + type-ahead + thinking display, unreleased — see [[changelog]])
