...
## Settings loading
`load_config()` at [config.py](../kbcode/config.py) `load_config:496` merges settings
...
- `persist_global_choice()` ([config.py](../kbcode/config.py) `persist_global_choice:393`) —
...
- `save_model_cache(provider, models)` at [config.py](../kbcode/config.py) `save_model_cache:488`
- `load_model_cache(provider)` at [config.py](../kbcode/config.py) `load_model_cache:474`
