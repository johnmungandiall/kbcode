...
- **Subagent** — specialist that runs in its own context window and returns a summary (`kbcode/subagents.py:76`) — see [[modes-subagents]]
...
- **FTS5** — SQLite full-text search, used by memory for `recall()` with LIKE fallback (`kbcode/memory.py:78`)
