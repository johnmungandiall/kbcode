...
- **Normalized message format** — provider-agnostic message shape used by the agent loop (`kbcode/agent.py:78`) — see [[providers]]
...
- **Compaction** — summarizing old chat turns to stay within context window (`kbcode/compaction.py:104`) — see [[context-management]]
- **Shadow git** — isolated git store for auto pre-edit snapshots (`kbcode/checkpoints.py:54`) — see [[safety]]
- **Standing orders** — always-on user instructions prepended to every session (`kbcode/cli.py:79` `_STANDING_ORDERS_TEMPLATE`) — see [[modes-subagents]]
