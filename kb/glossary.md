...
- **Normalized message format** — provider-agnostic message shape used by the agent loop (`kbcode/agent.py:92`) — see [[providers]]
...
- **Standing orders** — always-on user instructions prepended to every session (`kbcode/cli.py:111` `_STANDING_ORDERS_TEMPLATE`) — see [[modes-subagents]]
- **Subagent** — specialist that runs in its own context window and returns a summary (`kbcode/subagents.py:42`) — see [[modes-subagents]]
