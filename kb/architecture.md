...
## Data / control flow
1. `main()` (`kbcode/cli.py:381`) → `load_config()` → `_build_agent()` → `repl()` (`kbcode/repl.py:222`)
2. User types a message → `Agent.run()` (`kbcode/agent.py:248`) → `Agent._complete()` (`kbcode/agent.py:132`) calls provider
3. Provider returns text + tool_calls → agent loop dispatches through `Agent._dispatch_tool()` (`kbcode/agent.py:203`), which runs `PreToolUse`/`PostToolUse` hooks around `Tools.execute()` (`kbcode/tools/core.py:102`) — built-ins via `_tool_<name>` methods, `mcp__*` names via the MCP fork ([[mcp]]) — see [[safety]]
