...
folder's > the global `~/.kbcode` (`global_dir()`, `kbcode/config.py:308`) > preset
...
`PRESETS` (`kbcode/config.py:122`) is the source of truth for built-in providers
...
per server name — `load_mcp_servers()`, `kbcode/config.py:326`, carried as
`Config.mcp`, `kbcode/config.py:205`), unlike every other settings key where
...
`_ensure_self_ignore()` (`kbcode/config.py:295`) drops a `*` .gitignore inside it
(only if absent — a user-customized one is left alone; OSError swallowed), called
from `ensure_dirs()` (`kbcode/config.py:268`) and `save_settings()`
(`kbcode/config.py:351`). **Machine-local runtime state** — `memory_db`,
`sessions_dir`, `checkpoints_dir`, `history_file`, kbcode.log — hangs off
`Config.state_dir` (`kbcode/config.py:217`): `~/.kbcode/projects/<slug>/`,
...
(`kbcode/config.py:318`) encodes the resolved absolute path — every
...
mkdirs `global_dir()` right after `load_config` (`kbcode/cli.py:418`) — a
...
(`kbcode/config.py:200`) flows into both provider clients via
...
(`DEFAULT_MAX_STEPS = 50` / `DEFAULT_MAX_COMMANDS = 25`, `kbcode/config.py:116-117`),
...
The emergency context stop and `_SUBAGENT_MAX_STEPS` still apply. Hitting a
positive cap pauses the turn safely — "continue" resumes — see [[gotchas]] for
why real tasks hit them. Also documented in `.env.example` and the README
tuning-knobs section (lockstep: change the semantics → update both + [[safety]]).
...
from `kbcode/cli.py:418` right after `load_config`) attaches a rotating handler
