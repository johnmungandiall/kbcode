...
`SessionRecorder` (`kbcode/sessions.py:90`) appends one JSON line per message **as it
happens** — `Agent._append()` (`kbcode/agent.py:237`) wraps every
...
every turn), and `reset` (written by `Agent.reset()`, `kbcode/agent.py:840` — a marker,
...
.load_session`, `kbcode/sessions.py:257`) since `compaction._render()` reads them by
attribute.

## Usage tally
`Agent.usage` (`kbcode/agent.py:103`) accumulates `requests`/`input_tokens`/
`output_tokens` from each response's usage (both providers populate it, see
[[providers]]), written through `Agent._record_usage()` (`kbcode/agent.py:648`)
...
(`kbcode/agent.py:655`) + `pricing.estimate_cost()`
(`kbcode/pricing.py:27`) back `/insights`. After every user turn `Agent._turn_summary()`
(`kbcode/agent.py:635`) prints the `actions * tokens * elapsed` footer and records
usage so `/insights` reflects a session even if it never cleanly exits.
`sessions.lifetime_stats()` (`kbcode/sessions.py:335`) rolls every saved session's
