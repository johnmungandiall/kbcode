...
into a pending buffer (`kbcode/prompt_input.py:180` `_attach_image`); `repl.py` drains
it and calls `Agent.run(user, images=...)` (`kbcode/agent.py:182`) — the `images` key
...
`compaction.estimate_tokens()` (`kbcode/compaction.py:44`) counts each image as a flat
...
`_try_vision_fallback` (`kbcode/agent.py:140`), which asks `vision_fallback
...
local file (<=30MB). `/video <path>` (chat, `kbcode/cli.py:301` `_describe_videos`) /
`--video` (one-shot, `kbcode/cli.py:286`) call this synchronously; the description is
