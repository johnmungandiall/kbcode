...
| **Vision/video analyze tools** (`tools/vision_tools.py`) | Auxiliary vision-capable model describes an image/video when the main model can't see it natively (or, for video, has no native path at all), so a non-vision main model still gets to act on the media. | ✅ `vision_fallback.py` — auto-triggered on the "model doesn't support image input" `ProviderError` (images), always for `/video`/`--video` (`videos.py`, `cli.py`, `agent.py`). Right-sized: one auxiliary OpenAI-compatible client, no separate provider-profile registry like Hermes' full version. |
